...
public interface BookingRepository extends JpaRepository<tbl_Booking, Long> {
    ...
    @Query(nativeQuery = true, value = "exec pr_Pay @email =:email, @accountNo =:accountNo,@bookingId =:bookingId," + "@discount =:discount")
    String pay(@Param("email") String email, @Param("accountNo") String accountNo, @Param("bookingId") String bookingId, @Param("discount") Long discount);
...
//    List<tbl_Booking> findByBookingStatusAndEmail(int status,String email);
}
